...
def lassoAIC(Xtrain, Ytrain, reg_mod, unreg_mod, tol=1e-8, method="kawano", pos_class=1):
    ...
    if method == "kawano":
        ...
        Xtrain_const = sm.tools.add_constant(Xtrain, prepend=True)
        Xtrain_nonzero = sm.tools.add_constant(Xtrain_const)
...
def calcAICsLasso(penalty, Xtrain, Ytrain, sample_size, unreg_full_mod=None, rs=123):
    cur_mod_reg = glm(C=penalty, max_iter=10000, penalty="l1", solver='liblinear', random_state=rs, n_jobs=1).fit(Xtrain, Ytrain)
    ...
    if unreg_full_mod is not None:
        aics.append(lassoAIC(Xtrain, Ytrain, cur_mod_reg, unmod_reg, tol=1e-8, method="kawano"))
        aics.append(lassoAIC(Xtrain, Ytrain, cur_mod_reg, unmod_reg, tol=1e-8, method="hastie"))
